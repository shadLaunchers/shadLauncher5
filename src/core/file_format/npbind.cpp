...
std::vector<std::string> NPBindFile::GetNpCommIds() const {
    ...
    for (const auto& body : m_bodies) {
        // The npcommid field is a fixed-size, NUL-padded region (e.g. "NPWR22506_00"
        // followed by trailing zero bytes to fill out the entry). Building a
        // std::string from the raw buffer with its full length embeds those
        // trailing NULs *inside* the string rather than terminating it, which
        // then corrupts anything downstream that treats it as a normal C-style
        // string (list widgets, joined/newline-separated text, etc). Only take
        // the bytes up to the first NUL.
        if (!body.npcommid.data.empty()) {
            const char* raw = reinterpret_cast<const char*>(body.npcommid.data.data());
            size_t len = 0;
            while (len < body.npcommid.data.size() && raw[len] != '\0') {
                ++len;
            }
            npcommids.emplace_back(raw, len);
        }
    }
