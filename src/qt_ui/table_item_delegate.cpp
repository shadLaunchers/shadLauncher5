...
void TableItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
                              const QModelIndex& index) const {
    // Handle attribute fields in param.json viewer (value column, long text with commas)
    if (!m_has_icons && index.column() == 1) { // Value column in param.json viewer
...
QSize TableItemDelegate::sizeHint(const QStyleOptionViewItem& option,
                                  const QModelIndex& index) const {
    // Handle attribute fields in param.json viewer
    if (!m_has_icons && index.column() == 1) { // Value column in param.json viewer
        QString text = index.data(Qt::DisplayRole).toString();
