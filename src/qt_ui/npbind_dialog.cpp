...
// followed by trailing zero bytes). Converting the raw bytes to a QString
// with their full length embeds those trailing NULs *inside* the string
// instead of terminating it, which corrupts how Qt renders/wraps it (looks
// fine until you select a different body and the label garbles). Only take
