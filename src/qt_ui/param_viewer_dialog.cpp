...
bool ParamViewerDialog::loadParam(const QString& path) {
    ...
    // GetEntries() walks the document itself, so fields with no member on Param
    // -- and fields Sony adds later -- still show up here.
    ...
    for (const auto& e : flattened) {
        ...
        // Containers carry a synthesized "N item(s)" summary, not a real value,
        // so there is nothing for a decoder to act on.
...
void ParamViewerDialog::onExport() {
    ...
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Export"), suggested, tr("JSON (*.json);;CSV (*.csv);;HTML (*.html)"));
    if (fileName.isEmpty()) {
        return;
    }
    ...
    // JSON export: a flat path -> value map of the leaves only. Container rows
    // carry a synthesized "N item(s)" summary rather than a value, and the
    // nesting they describe is already in the Raw JSON tab and on disk.
    QJsonObject obj;
    for (const auto& e : m_model->entries()) {
        if (e.type == QStringLiteral("object") || e.type == QStringLiteral("array")) {
            continue;
        }
        obj[e.key] = e.value;
    }
...
void ParamViewerDialog::applyFilter(const QString& text) {
    ...
    if (m_regexCheck->isChecked()) {
        ...
        // Fall back to a literal match while the user is midway through typing
        // a pattern that isn't valid yet.
        if (!regex.isValid()) {
